...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    dev::eth::EVMSchedule schedule = uiddDGP.getGasSchedule(coinbaseMaturity + 900);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    UiddDGP uiddDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    dev::eth::EVMSchedule schedule = uiddDGP.getGasSchedule(coinbaseMaturity + 2); // After initializing the tests, the height of the chain 502
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    uint32_t blockSize = uiddDGP.getBlockSize(nHeight);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    UiddDGP uiddDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint32_t blockSize = uiddDGP.getBlockSize(coinbaseMaturity + 2);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    UiddDGP uiddDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint64_t minGasPrice = uiddDGP.getMinGasPrice(coinbaseMaturity + 2);
