...
public:
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_uiddDelegation.ExistDelegationContract() ? m_uiddDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_uiddDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_uiddDelegation.ExistDelegationContract() ? m_uiddDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_uiddDelegation.VerifyDelegation(address, delegation);
    ...
    UiddDelegation m_uiddDelegation;
};
