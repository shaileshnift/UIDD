...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 20000 * COIN / blocktimeDownscaleFactor;
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    UiddTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = UiddTransaction(nAmount, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = UiddTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(UiddTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, UiddTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        UiddTxConverter convert(tx, NULL, NULL, contractflags);
