...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // uidd
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            UiddTxConverter converter(tx, NULL);
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s UIDD additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateUidd";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ByteCodeExec exec(block, std::vector<UiddTransaction>(1, callTransaction), blockGasLimit);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // uidd
    UiddDGP uiddDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setUiddSchedule(uiddDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = uiddDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = uiddDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = uiddDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // uidd
...
///////////////////////////////////////////////////////////////////////////////////////// uidd
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            UiddTxConverter convert(tx, &view, &block.vtx);
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertUiddTX.first, blockGasLimit);
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertUiddTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertUiddTX.first[k].from(), resultConvertUiddTX.first[k].to(),
