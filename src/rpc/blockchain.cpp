...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//toniuidd edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // uidd
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // uidd
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // uidd
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // uidd
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress uiddSenderAddress(request.params[2].get_str());
        if(uiddSenderAddress.IsValid()){
            CKeyID keyid;
            uiddSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // uidd
