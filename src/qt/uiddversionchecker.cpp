...
#define paternVersion "uidd-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

UiddVersionChecker::UiddVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

UiddVersionChecker::~UiddVersionChecker()
{

}

bool UiddVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> UiddVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(UIDD_RELEASES)));
    QEventLoop event;
...
Version UiddVersionChecker::getMaxReleaseVersion()
{
